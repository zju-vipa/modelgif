...
def load_model(num, mode):
    if mode == 'teacher':
        ...
        if num < 5:
            ...
            model.classifier[-1] = torch.nn.Linear(in_feature, 10)
        model.load_state_dict(
            torch.load(os.path.join("model", "student_model_1_" + str(num) + ".pth")))
        ...
        if num < 3:
            ...
            model.fc = torch.nn.Linear(in_feature, 10)
        model.load_state_dict(
            torch.load(os.path.join("model", "student_model_tk_" + str(num) + ".pth")))
        ...
        model = torch.load("Fine-Pruning/prune_model_" + str(num) + ".pth", map_location="cpu")
