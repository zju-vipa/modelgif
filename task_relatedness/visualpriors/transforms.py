...
class VisualPrior(object):
    ...
    viable_feature_tasks = [
        'autoencoding', 'colorization', 'curvature', 'denoising',
        'edge_texture', 'edge_occlusion', 'egomotion', 'fixated_pose',
        'jigsaw', 'keypoints2d', 'keypoints3d', 'nonfixated_pose',
        'point_matching', 'reshading', 'depth_zbuffer', 'depth_euclidean',
        'normal', 'room_layout', 'segment_unsup25d', 'segment_unsup2d',
        'segment_semantic', 'class_object', 'class_scene', 'inpainting',
        'vanishing_point'
    ]
    ...
    def to_predicted_label(cls,
        ...
        with torch.no_grad():
            ...
            for net in nets:
                tempa = net(
                    img)  # img:(tensor) 1,3,256,256 tempa：(tensor) 1,3,256,256
            return torch.cat([net(img) for net in nets], dim=1)
    ...
    def to_get_net(cls,
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for net in nets:
                    temp = 1
                    # tempa = net(img)  # img:(tensor) 1,3,256,256 tempa：(tensor) 1,3,256,256
                    # return torch.cat([net(img) for net in nets], dim=1)
                    return net
        except:
            VisualPriorRepresentation._load_unloaded_nets(feature_tasks)
        ...
        for feature_task in feature_tasks:
            if feature_task not in cls.feature_task_to_net:
                if feature_task not in TASKS_TO_CHANNELS:
                    raise NotImplementedError(
                        'Task {} not implemented in VisualPriorPredictedLabel. Recommended to get predictions from {}'
                        .format(feature_task, TASKONOMY_LOCATION))
